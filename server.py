...
SLOTS=['weapon','helm','armor','legs','gloves','boots','relic']
SETS={
...
PREFIX={'weapon':['Rending','Chromatic','Overclocked','Voltaic','Celestial','Siegeborn'], 'helm':['Oracle','Circuit','Specter','Vanguard','Halo','Aegis'], 'armor':['Bastion','Reactive','Riftwoven','Titan','Nova','Gravitic'], 'legs':['Strider','Bulwark','Riftguard','Gravitic','Phase','Titan'], 'gloves':['Striker','Pulse','Riftgrip','Phasebound','Impact','Arc'], 'boots':['Vector','Warp','Comet','Ghost','Riftstep','Kinetic'], 'relic':['Nexus','Vortex','Luminous','Abyssal','Prism','Singularity']}
BASE_STAT={'weapon':'damage','helm':'crit','armor':'hp','legs':'armor','gloves':'rate','boots':'speed','relic':'cooldown'}
...
def gear_look(uid,cls):
 ...
 for g in eq:
  ...
  variant=h%6
  slots[g['slot']]={'rarity':RARITIES.index(g['rarity']) if g['rarity'] in RARITIES else 0,'set':g.get('set_name') or '','name':g['name'],'variant':variant,'id':g['id']}
...
class Player:
    def __init__(self, pid, ws, name, cls, uid=None):
        ...
        self.dmg=0;self.kills=0;self.deaths=0;self.highest=0;self.level=1;self.xp=0;self.items={};self.choice=[];self.crit=0;self.ranks={};self.perm={};self.proc=0;self.drone_at=0;self.orbit_at=0;self.downed=False;self.down_until=0;self.revive_progress=0;self.dash_until=0;self.dash_dx=0;self.dash_dy=-1;self.dash_speed=580
    def snapshot(self,now):
        return {'id':self.id,'name':self.name,'cls':self.cls,'team':self.team,'ready':self.ready,'x':round(self.x,1),
          'y':round(self.y,1),'angle':round(self.angle,3),'move':[round(float(self.cmd.get('dx',0)),3),round(float(self.cmd.get('dy',0)),3)],'hp':round(self.hp,1),'maxHp':self.max_hp,'shield':round(self.shield,1),
          ...
          'cd':{k:round(max(0,v-now),1) for k,v in self.cd.items()},'dashCD':round(max(0,self.last_dash+max(2.2,4.5-(.32*self.items.get('swiftstep',0) if self.cls=='duelist' else 0))-now),1),'dashFor':round(max(0,self.dash_until-now),3),'dashDir':[round(self.dash_dx,3),round(self.dash_dy,3)],'moveSpeed':round(SPEED[self.cls]*item_speed_mult(self)*(1+self.gear.get('speed',0)*.006),1),'loadout':self.loadout,'basic':self.basic,'look':gear_look(self.uid,self.cls) if self.uid else {}}
...
class Room:
    def __init__(self, code, mode, host):
        ...
        self.state='lobby';self.started=0;self.round=0;self.wins=[0,0];self.next_round=0;self.enemies={};self.events=[];self.projectiles={};self.proj_seq=0;self.pickups={};self.pickup_seq=0
    ...
    def packet(self,now):
        return {'type':'state','code':self.code,'mode':self.mode,'phase':self.state,'host':self.host,'round':self.round,
                ...
                'pickups':[{'id':q['id'],'x':round(q['x'],1),'y':round(q['y'],1),'owner':q['owner'],'quality':q['quality'],'life':round(max(0,q['expires']-now),1)} for q in self.pickups.values()],
                'events':self.events[-60:],'score':round(self.score),
    ...
    def begin(self,now):
        self.assign_teams();self.state='playing';self.started=now;self.round=1;self.wins=[0,0];self.score=0;self.enemies.clear();self.projectiles.clear();self.pickups.clear();self.events.clear();self.spawn=now+.5;self.boss_mark=1500;self.boss_count=0;self.hazards=[];self.wave=1;self.wave_mult=1.;self.wave_floor=1.;self.wave_quota=13;self.wave_spawned=0;self.wave_killed=0;self.wave_started=now;self.wave_transition=0
        self.prepare_round(now)
    def prepare_round(self,now):
        self.enemies.clear();self.projectiles.clear();self.pickups.clear();self.next_round=now+1.6
        ...
        for p in self.players.values():
            ...
            p.max_hp=HEALTH[p.cls]*(1+p.gear.get('hp',0)*.01+p.perm.get('hp',0));p.hp=p.max_hp;p.shield=0;p.alive=True;p.downed=False;p.down_until=0;p.revive_progress=0;p.dash_until=0;p.cd.clear();p.last_shot=now;p.last_dash=now-10;p.invul_until=now+1.0
    ...
    def hit(self,target,attacker,raw,now,kind='hit'):
        ...
        if target.hp<=0:
            target.hp=0
            if isinstance(target,Player):
                can_revive=((self.kind=='pve' or self.mode=='teams') and len(self.players)>1)
                if can_revive:
    ...
    def hurt_enemy(self,e,p,raw,now):
        ...
        if e['hp']<=0:
            ...
            if ENEMY_REGISTRY[e['kind']]['role']=='boss':
                ...
                self.spawn_augment_pickup(p,e['x'],e['y'],now,'boss')
            elif e.get('wave')==self.wave:
                self.wave_killed+=1
                if e['elite'] and self.rng.random()<.30:self.spawn_augment_pickup(p,e['x'],e['y'],now,'elite')
                elif p.kills%12==0:self.spawn_augment_pickup(p,e['x'],e['y'],now,'normal')
            need=int(35*1.3**(p.level-1))
            if p.xp>=need and not p.choice:
                p.xp-=need;p.level+=1;pool=self.augment_pool(p,'normal');p.choice=self.rng.sample(pool,min(3,len(pool)))
    ...
    def tick_revives(self,now,dt):
        # Revives: Endless Duo/Trio/Squad + 2v2. Never 1v1.
        revivable=((self.kind=='pve' or self.mode=='teams') and len(self.players)>1)
        ...
        for down in self.players.values():
            ...
            helpers=[p for p in self.players.values() if p.alive and not p.downed and p.id!=down.id and (self.kind=='pve' or p.team==down.team) and p.cmd.get('revive') and math.hypot(p.x-down.x,p.y-down.y)<=60]
            if helpers:
                down.revive_progress=min(1.55,down.revive_progress+dt*(1+.22*(len(helpers)-1)))
                if int((down.revive_progress-dt)*10)!=int(down.revive_progress*10):self.event('revive_tick',down.x,down.y,team=down.team,progress=round(down.revive_progress,2))
            else:down.revive_progress=max(0,down.revive_progress-dt*.22)
            if down.revive_progress>=1.55:
                down.downed=False;down.alive=True;down.hp=max(1,down.max_hp*.45);down.shield=min(40,down.shield+22);down.invul_until=now+1.6;down.revive_progress=0
                self.event('revive',down.x,down.y,name=down.name,team=down.team)

    def augment_pool(self,p,quality='normal'):
        pool=[x for x in ITEMS if p.items.get(x,0)<8 and (x not in ITEM_CLASS or ITEM_CLASS[x]==p.cls)]
        if quality=='boss':
            priority={'storm','orbit','drone','critboom','glass','shrapnel','doubletap','targeter','blooddance','specterblade','swiftstep','tremor','jugger','ironwave','flux','riftspark','manashield'}
            rich=[x for x in pool if x in priority]
            if len(rich)>=3:pool=rich
        return pool

    def spawn_augment_pickup(self,p,x,y,now,quality='normal'):
        if self.kind!='pve' or not p:return
        self.pickup_seq+=1
        a=self.rng.random()*math.tau;d=self.rng.uniform(4,16)
        q={'id':self.pickup_seq,'x':x+math.cos(a)*d,'y':y+math.sin(a)*d,'owner':p.id,'quality':quality,'expires':now+(38 if quality=='boss' else 30)}
        self.pickups[q['id']]=q
        self.event('augment_drop',q['x'],q['y'],player=p.id,quality=quality)

    def tick_pickups(self,now):
        if self.kind!='pve':return
        for qid,q in list(self.pickups.items()):
            if now>=q['expires']:
                self.pickups.pop(qid,None);continue
            p=self.players.get(q['owner'])
            if not p or not p.alive or p.choice:continue
            if math.hypot(p.x-q['x'],p.y-q['y'])<=27:
                pool=self.augment_pool(p,q['quality'])
                if pool:
                    p.choice=self.rng.sample(pool,min(3,len(pool)))
                    self.event('augment_open',q['x'],q['y'],player=p.id,quality=q['quality'])
                self.pickups.pop(qid,None)
    ...
    def start_player_dash(self,p,d,distance,now):
        mag=math.hypot(d[0],d[1]) or 1
        p.dash_dx=d[0]/mag;p.dash_dy=d[1]/mag;p.dash_speed=580
        p.dash_until=now+distance/p.dash_speed
        p.invul_until=max(p.invul_until,p.dash_until+.10)
        self.event('dash',p.x,p.y,team=p.team,angle=math.atan2(p.dash_dy,p.dash_dx),r=18)
    ...
    def tick(self,now,dt):
        ...
        for p in list(self.players.values()):
            ...
            speed=SPEED[p.cls]*item_speed_mult(p)*(1+p.gear.get('speed',0)*.006)
            p.angle=num(cmd.get('angle'),0,-10,10)
            dash_cd=max(2.2,4.5-(.32*p.items.get('swiftstep',0) if p.cls=='duelist' else 0))
            if cmd.get('dash') and now>=p.last_dash+dash_cd and now>=p.dash_until:
                p.last_dash=now;d=(vx,vy) if math.hypot(vx,vy)>.1 else p.last_move;self.start_player_dash(p,d,84,now)
                if p.items.get('dashmine'):self.splash(p,p.x,p.y,50,DAMAGE[p.cls]*(.45+.18*p.items.get('dashmine',0)),now,'mine')
            if now<p.dash_until:
                step=p.dash_speed*dt;nx=p.x+p.dash_dx*step;ny=p.y+p.dash_dy*step
                if not solid(nx,p.y):p.x=nx
                else:p.dash_until=now
        ...
        if self.kind=='pvp':
            active=[sum((p.alive or (self.mode=='teams' and p.downed)) and p.team==team for p in self.players.values()) for team in (0,1)]
            if 0 in active:
        ...
        async for msg in ws:
            ...
            if typ=='hub_input':
                ...
                if h:
                    ...
                    if now-h.get('broadcast',0)>.033:h['broadcast']=now;await push_hub()
                continue
            if typ=='hub_class':
            ...
            if typ=='ready' and room.state=='lobby':
                ...
                now=time.monotonic()
                if now-last_input<.018:continue
                last_input=now
...
async def health(request):return web.json_response({'status':'ok','rooms':len(ROOMS),'version':'pixel-online-0.7.8-animation-gearfit'})
app=web.Application()
