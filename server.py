        ...
        if e['hp']<=0:
            ...
            if p.uid:
                ...
                if ENEMY_REGISTRY[e['kind']]['role']=='boss' or self.rng.random()<(.08 if e['elite'] else .012):
                    loot=None
                    if len(owned_gear(p.uid))<160:loot=make_gear(p.uid,p.cls,'Legendary' if ENEMY_REGISTRY[e['kind']]['role']=='boss' else None)
                    if loot:
                        asyncio.create_task(self.send(p,{'type':'gear_drop','name':loot['name'],'rarity':loot['rarity'],'slot':loot['slot']}))
...
async def health(request):return web.json_response({'status':'ok','rooms':len(ROOMS),'version':'pixel-online-0.7.2-crucible-district'})
app=web.Application()
